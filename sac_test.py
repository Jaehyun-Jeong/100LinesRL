...
lr_pi, lr_q, lr_alpha, gamma, batch_size, buffer_limit, tau = 0.0005, 0.001, 0.001, 0.98, 32, 50000, 0.01
init_alpha, target_entropy = 0.01, -1.0
...
def calc_target(pi, q1_t, q2_t, mb):
    with T.no_grad():
        a_prime, log_prob = pi(mb['sp'])
       return mb['r'] + gamma * mb['done'] * (T.min(q1_t(mb['sp'], a_prime), q2_t(mb['sp'], a_prime)) - pi.log_alpha.exp() * log_prob)

def main():
    env, memeory = gym.make('Pendulum-v1'), ReplayBuffer()
    q1, q2, q1_t, q2_t, pi = QNet(), QNet(), QNet(), QNet(), PolicyNet()
